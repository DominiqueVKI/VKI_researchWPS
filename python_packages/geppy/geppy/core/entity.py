...
from ..core.symbol import Function, RNCTerminal, Terminal, PowTerminal
...
#        for p in expr:
#            print(p)
        # this variable is always null this is strange
		...
		#if isinstance(expr[0], PowXTerminal):  # only contains a single RNC, let's retrieve its value
        #    return str(self.rnc_array[self.dc[0]])
    ...
    def kexpression(self):
        ...
        def convert_POW(p):
            ...
            if isinstance(p, RNCTerminal):
                ...
                t = Terminal(str(value), value)
                return t
            if isinstance(p, PowTerminal):
                index = self.dc[n_rnc]
                value = self.rnc_array[index]
                n_rnc += 1
                t = Terminal(p.name+str(value), value)
                return t
            return p
        ...
        expr = KExpression([convert_POW(self[0])])
        i = 0
        j = 1
        while i < len(expr):
            for _ in range(expr[i].arity):
                #expr.append(convert_RNC(self[j]))
                expr.append(convert_POW(self[j]))
                j += 1
            i += 1
        return expr
